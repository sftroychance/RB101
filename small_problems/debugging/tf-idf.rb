# The problem with the original code is that when calling Math#log,
# it is sending an integer value, the result of dividing the
# two values. Looking at the output compared to the desired output,
# only values below a certain threshold were set to 0.0, which indicated
# a possible math issue. Casting one of the values in the equation
# to a float results in a float value being sent to Math#log, and this
# indeed fixes the problem.
#
# The official solution indicated a particular problem in which Infinity
# or NaN were results from the calculation, which results when the
# value of number_of_documents_with_term == 0 (dividing by zero), so
# we can set the calculation result to 0.0 when that happens.
#
